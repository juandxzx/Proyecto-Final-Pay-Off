...
#seccion clientes-------------------------------------------------------------------------------------------------------------------------------------
        ...
        fact_creadores = Label(details, text="Powered By:\n Juan Carreño - Andrés Zorro", font=(
            "Tw Cen MT Condensed Extra Bold", 14), fg="green").grid(row=0, column=8, padx=10)
...
#factura---------------------------------------------------------------------------------------------------------------
        ...
        factuarea = Frame(self.root, bd=10, relief=GROOVE, bg="#3891c8")
        factuarea.place(x=1010, y=188, width=330, height=372)

        factu_title = Label(factuarea, text="Área de Facturación", font=(
            "Arial Black", 17), bd=7, bg="#3891c8", fg="#ffffff").pack(fill=X)

        scrol_y = Scrollbar(factuarea, orient=VERTICAL)
        self.txtarea = Text(factuarea, yscrollcommand=scrol_y.set)
...
#parte inferior fact---------------------------------------------------------------------------------------------------------------
        ...
        factur_menu=LabelFrame(self.root, text="Facturaciòn individual", font=("Tw Cen MT Condensed Extra Bold", 12), relief=SUNKEN, bd=10, fg="black")
...
def total(self):
    ...
    self.total_speakers.set(str(total_speakers_valor)+" COP")
    self.b.set(str(round(total_speakers_valor*0.1, 3))+" COP")
    ...
    self.total_accesorios.set(str(total_accesorios_valor)+" COP")
    self.c.set(str(round(total_accesorios_valor*0.07, 3))+" COP")
    self.total_all_fact = (total_celulares_valor +
                           total_speakers_valor +
                           total_accesorios_valor +
                           (round(total_accesorios_valor*0.1, 3)) +
...
def intro(self):
    ...
    self.txtarea.insert(END, f"\nNombre Cliente : {self.nombre_c.get()}")
...
def factuarea(self):
    ...
    if self.b.get() != "0.0 COP":
        self.txtarea.insert(END, f"Total IVA Speakers: {self.b.get()}\n")
    if self.c.get() != "0.0 COP":
        self.txtarea.insert(
            END, f"Total IVA Accesorios: {self.c.get()}\n")
    self.txtarea.insert(END, f"Monto total facturado : {self.total_all_fact}\n")
    self.txtarea.insert(END, f"------------------------------------\n")
...
datos_facturas = {}
datos_facturas['facturas'] = []

datos_facturas['facturas'].append({
    f"\n\nFactura Número. : {self.fact_no.get()}",
    f"\nNombre Cliente : {self.nombre_c.get()}",
    f"\nTeléfono : {self.telef.get()},
    f"Monto total facturado : {self.total_all_fact}\n"})


with open('datos_fact.json', 'w') as file:
